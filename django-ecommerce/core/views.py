...
def add_url_params(camp_id):
    ...
    url_params = '&'.join([f'{k}={v}' for k, v in tracking_params.items()])
    return url_params
...
def add_to_cart(request, slug):
    ...
    url_params = add_url_params('add2cart')
    if order_qs.exists():
        order = order_qs[0]
        ...
        if order.items.filter(item__slug=item.slug).exists():
            ...
            return redirect("/order-summary/?{0}".format(url_params))
        else:
...
def remove_from_cart(request, slug):
    ...
    url_params = add_url_params('rem4mcart')
    if order_qs.exists():
        order = order_qs[0]
        # check if the order item is in the order
        if order.items.filter(item__slug=item.slug).exists():
...
def add_to_favourites(request, slug):
    ...
    url_params = add_url_params('add2fav')
    ...
    if item:
        ...
        # , slug=slug)
        return redirect("/product/{0}?{1}".format(slug, url_params))
    else:
        messages.error(
