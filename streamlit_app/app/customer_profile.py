...
form_sub = False
with st.form(key="cust_id"):
    selected_customer = st.text_input("Customer ID")
    if st.form_submit_button() and selected_customer != '':
        form_sub = True
...
if form_sub:
    ...
    with tab1:
        ...
        st.write("Birth Date:",selected_row["birth_date"].dt.date.values[0])
    ...
    with tab2:
        ...
        with st.expander("Add to Cart", expanded=False):
    ...
    with tab3:
        ...
        with st.expander("Top 10 Products", expanded=False):
            st.bar_chart(data=top_10_prod_df.sort_values(by = 'total_quantity', ascending  = False), x="product_name", y="total_quantity")

        with st.expander("Top 10 Product Categories", expanded=False):
            st.bar_chart(data=top_10_prodcat_df.sort_values(by = 'total_quantity', ascending  = False), x="product_subcategory", y="total_quantity")
    ...
    with tab4:
        ...
        def create_graphviz_chart(segment_df):
            ...
            customer = list(segment_df['full_name'].unique())[0]
            segments = list(segment_df['segment_name'])
            for segment in segments:
                chart.edge(customer, segment)
